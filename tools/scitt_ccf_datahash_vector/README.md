...
| D | indefinite outer array, derived here | 166 B | digest matches `vektor_d.json`, signature taken over from A and verified with the published public key |
...
### Inputs to ToBeSigned

RFC 9052 §4.4 builds ToBeSigned from four inputs: the context string `Signature1`, the protected
header, `external_aad` and the payload. Measured per case by `nachrechnen.py` and recorded under
`tobesigned_inputs` in `nachrechnung.json`:

| case | payload in the envelope | type | length | payload sha256 | external_aad | signature |
|---|---|---|---|---|---|---|
| A | embedded | bstr | 59 B | `e2769722a4882da3c4513fde12e9aa020fac5450c26938795abe7e696b10df64` | empty | reproduced byte for byte from the published seed |
| B | embedded | bstr | 59 B | same | empty | reproduced byte for byte from the published seed |
| C | embedded | bstr | 59 B | same | empty | reproduced byte for byte from the published seed |
| D | embedded | bstr | 59 B | same | empty | taken over from A when D was minted here, verified with the published public key |

`external_aad` is not carried by a COSE_Sign1. The recomputation supplies the empty byte string.
Ed25519 is deterministic, so a signature regenerated byte for byte from the published seed shows
that the signed Sig_structure had exactly these inputs, the empty `external_aad` and the embedded
payload included. The seed and the signatures are the vector author's material, the
recomputation is ours. D's signature was copied from A, so regenerating it would regenerate A's;
D is therefore evidenced by verification.

Two counter-probes on A, recorded in the same file:

- `external_aad_probe_on_A`: with one byte `00` as `external_aad` the signature does not verify,
  with the empty one it does.
- `A_detached`: A with the payload element replaced by nil (`0xf6`), 105 B, sha256
  `388100857dbd87dabdcf65bccc9ab4bd142aa8fbd0d63935a5f767a4b13a7982`. From this envelope alone
  no Sig_structure can be formed, because the payload element is nil. With A's payload supplied
  from outside, the Sig_structure is byte-identical to A's (`60b4c76b…`) and the unchanged
  signature verifies.
...
## Reproducing
    ...
    python3 nachrechnen.py              # recomputes all four published states and D, compares the
                                        ...
                                        # records the ToBeSigned inputs and the two probes on A
