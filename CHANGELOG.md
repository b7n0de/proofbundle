...
- **A diff is read in git's grammar, by one parser, and judged in Python's lines**
  ...
  symlink or a gitlink under `src/proofbundle`, or `src` or `src/proofbundle` itself as one, is now a
  finding of the guard's own: the diff shows a link's text or a commit id and Python runs what it
  points at, so a mutant planted outside the security path was reported clean with exit 0 (measured
