...
python examples/inspect_receipt.py
echo
echo "-- lm-evaluation-harness (dummy) results  ->  signed receipt  ->  verify --"
python examples/lm_eval_receipt.py
echo
...
python -c "from proofbundle import verify_bundle; from proofbundle.emit import emit_bundle, generate_signer; \
