...
def run(fixture_dir: pathlib.Path, out_dir: pathlib.Path) -> dict[str, Any]:
    ...
    # Enkodieren darf die stdlib, DEKODIEREN nicht: `payload` ist eine DSSE-Huelle, und
    # dort gilt genau eine Drahtform. Siehe src/proofbundle/_wire_b64.py.
    ...
    dec_payload = decode_b64_either(dec_env["payload"])
...
def verify_pair(report: dict[str, Any]) -> dict[str, Any]:
    ...
    # Oeffentliche Schluessel aus einer Berichtsdatei sind unvertraute Drahtbytes: Standard-
    # alphabet, Padding verlangt, kanonisch. Die stdlib verwirft stattdessen still.
