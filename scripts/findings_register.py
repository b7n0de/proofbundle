...
def _resolve_current(findings: list) -> tuple[dict, list]:
    """current-wins: later duplicate ids and ``superseded_by`` links collapse to the effective entry.
    Returns (effective_by_id, contradictions). A contradiction is the same id appearing twice with a
    DIFFERENT status and no supersession link declaring a winner."""
    ...
    for f in findings:
        ...
        if isinstance(sby, str) and sby:
            superseded.add(f.get("id"))
    for f in findings:
        if not isinstance(f, dict):
            continue
        fid = f.get("id")
        if not isinstance(fid, str) or fid in superseded:
            continue
        if fid in effective and effective[fid].get("status") != f.get("status"):
            contradictions.append(fid)
        effective[fid] = f
    return effective, contradictions
...
def verify_and_count(repo: Path | str = REPO) -> dict:
    ...
    if not isinstance(findings, list) or not findings:
        return {"ok": False, "reason": "register lists no findings (RT-10 evaluated_count==0 -> FAIL)",
                "open_ids": [], **triple, "source_digest": source_digest}
    effective, contradictions = _resolve_current(findings)
    ...
    open_p0p1 = sorted(fid for fid, f in effective.items()
                       if f.get("severity") in _GATING_SEVERITIES and f.get("status") != "closed")
    ok = not open_p0p1
