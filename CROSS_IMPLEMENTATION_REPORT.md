...
**A MEASURED SUBSET of the corpus is reproduced independently, not all of it** (`crosscheck.py`, exit 0 — 14/14 on the recorded 3.2.0-era run this report documents). **Re-measured 2026-09-08 by running the tool: 57 of 107 corpus cases**, 42 of them relation vectors compared differentially (Python == Rust on exit class and lineage); the remainder is declared Python-only in the scope section below. The sentence here previously read "All corpus cases are reproduced" and then "56/56 as of v3.7.0" — two claims that were each true of a smaller, older corpus and together read as full coverage of the current one. A lens of the closing round found it; the number now carries the command that produces it and the date it was taken. Research note:
