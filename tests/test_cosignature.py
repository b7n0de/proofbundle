...
class TestCosignRoundtrip(unittest.TestCase):
    ...
    def test_quorum_dedups_by_key_not_name(self):
        # HIGH (release review 2026-07-02): one physical witness key registered under N different names must
        # count as ONE witness — else a single compromised key stuffs any threshold=N quorum, defeating the
        # split-view resistance that is the whole point of witnessing (C2SP: distinct keys per cosigner).
        log_key = generate_signer()
        note = cp.sign_checkpoint(ORIGIN, 7, ROOT, log_key, ORIGIN)
        log_vkey = cp.vkey(ORIGIN, _raw_pub(log_key))
        sole = generate_signer()            # ONE physical key, two names
        ...
        # both cosignature lines still individually verify (they are real signatures) — the dedup is in the count
        self.assertEqual(sum(1 for r in res["witnesses"].values() if r["ok"]), 2)
