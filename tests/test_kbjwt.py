...
class TestBundleIntegration(unittest.TestCase):
    ...
    def test_bundle_cnf_bound_stripped_kb_fails(self):
        # CRITICAL (release review 2026-07-02): a credential ISSUED WITH a cnf holder key REQUIRES
        # proof-of-possession. Stripping the KB-JWT down to the RFC-9901-legal no-key-binding form must
        # FAIL — else anyone who sees the disclosed SD-JWT replays it as a bearer token (the exact bypass
        # that the pre-fix code let verify OK because it only checked a KB-JWT that was still attached).
        presented, issuer, _ = _issue_presented()
        stripped = presented.rsplit("~", 1)[0] + "~"   # cut the KB-JWT → no-key-binding compact form
