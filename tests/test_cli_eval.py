...
class TestExpectIssuer(unittest.TestCase):
    """--expect-issuer (2026-08-23, from the adversarial re-check): without pinning, a claim
    RE-SIGNED with a fresh key passes show-eval rc=0. Pinning the issuer closes that gap."""

    def _receipt(self, d, name="receipt.json", key="k.key"):
        import os
        claim = os.path.join(d, f"claim_{name}")
        Path(claim).write_text(json.dumps({
            "schema": "proofbundle/eval-claim/v0.1", "suite": "s", "suite_version": "v1",
            "metric": "acc", "comparator": ">=", "threshold": "0.80", "passed": True, "n": 100,
            "model_id_commit": "sha256:x", "dataset_id_commit": "sha256:y",
    ...
    def test_matching_issuer_passes(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            r = _run("show-eval", out, "--expect-issuer", issuer)
            self.assertEqual(r.returncode, 0, r.stderr)

    def test_resigned_forgery_is_caught(self):
        """The exact adversarial scenario: a second receipt signed by a DIFFERENT key claims to
        be from the first issuer — without pinning it passes, with pinning it fails."""
        import tempfile
        with tempfile.TemporaryDirectory() as d:
            _, issuer_a = self._receipt(d, "a.json", "ka.key")
            forged, _ = self._receipt(d, "b.json", "kb.key")
            self.assertEqual(_run("show-eval", forged).returncode, 0)   # self-attested: passes
            r = _run("show-eval", forged, "--expect-issuer", issuer_a)
            self.assertEqual(r.returncode, 1)
            self.assertIn("issuer mismatch", r.stderr)

    def test_rotation_any_of_multiple_pins(self):
        import tempfile
        with tempfile.TemporaryDirectory() as d:
            out, issuer = self._receipt(d)
