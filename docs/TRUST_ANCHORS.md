...
| **Log key** (`verify_tlog_proof(log_vkey=…)`, checkpoint vkey) | OUT-OF-BAND | Supply the log's C2SP vkey and, optionally, the origin — `expected_origin` in the library, `verify-proof --expected-origin` on the command line (since 3.8.0). The comparison is exact: a prefix, a different case or a trailing slash is a mismatch. | A validly-signed checkpoint from an *unexpected* log would be accepted — pass the origin. |
