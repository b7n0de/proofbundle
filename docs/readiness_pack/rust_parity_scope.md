...
At v3.7.0 the conformance corpus held 57 cases and the `make conformance-crossimpl` gate reproduced 56 of them independently by the Rust binary. **That figure is bound to that corpus, not to this one.** The corpus holds 135 cases today, and the Rust build the previous wording said this needed has now been run: `cd tools/pb_verify_rs && cargo build --release` followed by `PYTHONPATH=src python tools/pb_verify_rs/crosscheck.py`, on this tree, first on 2026-09-20 over 130 cases and again on 2026-09-25 over 135.

MEASURED 2026-09-25: **61 of 135** corpus cases are reproduced independently, 45 of them relation vectors compared differentially on exit class and lineage. The gate names the remainder itself rather than folding it into the ratio — 74 cases are NOT RUN DIFFERENTIALLY, by kind: `agent_review_predicate` (35), `cap1_document` (15), `envelope_profile_rule` (14), `provenance_version_status` (10). The five cases added since 2026-09-20 are envelope-profile vectors, a kind this differential names rather than runs, so the reproduced count did not move. Run it and it prints that block before the verdict.

So the honest statement is neither the old "the whole corpus" nor "not measured": **55 per cent of the corpus is not differentially reproduced, and every one of those cases is accounted for by kind with a reason.** (Until 2026-09-25 this said 47 per cent over 130 cases. 47 was the reproduced share; the share not reproduced was 69 of 130, 53 per cent. A percentage and its complement read the same in a sentence and are not the same claim.) The largest single item, `cap1_document`, is not a missing Rust side — `tools/cap1_unabhaengige_umsetzung/rs/` exists and carries the same nine rules — but a Rust side that is not wired to this differential.
