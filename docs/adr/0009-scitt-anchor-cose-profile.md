...
- A key is built by its type, never by a generic loader (owner decision A1, 2026-09-26). The
  SubjectPublicKeyInfo of a relying-party key, and of an `x5chain` end-entity certificate, is read
  as DER first: an EC key on P-256 or P-384 is built from its point, an RSA key under rsaEncryption
  with NULL parameters from its modulus and exponent. Every other type (Ed25519, Ed448, X25519,
  X448, DSA, other curves, RSA under the id-RSASSA-PSS OID) and every SPKI that is not DER is
  refused while it is still bytes: a relying-party key of that type is ignored, as absent trust, and
  an end-entity key of that type selects nothing. Measured before this rule: `load_der_public_key`
  and `Certificate.public_key()` built each of these as a live key object, and the relying-party
  path kept every one of them as a usable key; the type was checked only at verification.
...
| algorithm and key binding | the protected alg, the RP key type and curve belong together | real control ES384 over P-384; real production receipt verified with a store entry labelled `ES256` (label ignored) | ES384 label with a P-256 RP key; ES256 label with a P-384 key; an EC label with an RSA key; an unsupported alg (outside profile, not invalid); ECDSA signature of wrong length; PS256 with a different salt length (synthetic); a key of each type v1 does not verify, as a relying-party key and as an `x5chain` end-entity key, refused before a key object exists (synthetic, `test_scitt_ccf_keys_are_built_by_type.py`) |
