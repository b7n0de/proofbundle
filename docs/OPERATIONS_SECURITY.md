...
Items marked **[Owner]** are operational actions only the maintainer can perform and verify (account
settings, registrar locks, key custody); this document does not assert they are done — it records what
the posture should be so it can be audited. Items marked **[repo]** are enforced by files in this
repository and are reviewable in a PR.

## Accounts and identity

- **[Owner]** GitHub account and the `b7n0de` org on 2FA with a **hardware security key** (WebAuthn),
  recovery codes stored offline. No SMS 2FA.
- **[Owner]** PyPI account for the `proofbundle` project on 2FA (hardware key), and the account that
  can publish is separate from day-to-day use where practical.
- **[Owner]** Org member access reviewed; no standing admin beyond the maintainer (see
  [oss_maintainer_roles.json](../oss_maintainer_roles.json), DEFAULT DENY).

## Release integrity

- **[Owner]** Publish to PyPI via **Trusted Publishing (OIDC)** where possible, so no long-lived API
...
- **[Owner]** Tags are **protected** (only the maintainer can create release tags) and every release
...
- **[Owner]** Private keys live only where the maintainer controls them (local, 0600, or a hardware/KMS
...
- **[Owner]** An OpenSSF **Scorecard** run is enabled and its badge reflects the real score, not an
  aspirational one.

## Domain and predicate URIs

- **[Owner]** `b7n0de.com` (which serves the vendored predicate URIs, e.g.
